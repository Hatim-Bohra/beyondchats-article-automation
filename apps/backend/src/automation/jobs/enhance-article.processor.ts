...
export class EnhanceArticleProcessor extends WorkerHost {
  ...
  async process(
    job: Job<EnhanceArticleJobData>,
  ): Promise<EnhanceArticleJobResult> {
    ...
    try {
      ...
      if (searchResults.length < 2) {
        this.logger.warn(
          `Only found ${searchResults.length} search results, proceeding anyway`,
        );
      }
      ...
      const scrapedContents = await Promise.all(
        searchResults.map((result) =>
          this.contentScraperService.scrapeArticle(result.url),
        ),
      );
      ...
      const finalContent = this.llmService.addReferences(
        enhancedContent,
        references,
      );
      ...
      this.logger.error(
        `Enhancement failed for article ${articleId}: ${error.message}`,
        error.stack,
      );
  ...
  onCompleted(job: Job<EnhanceArticleJobData>) {
    this.logger.log(
      `Job ${job.id} completed for article ${job.data.articleId}`,
    );
  }
  ...
  onFailed(job: Job<EnhanceArticleJobData>, error: Error) {
    this.logger.error(
      `Job ${job.id} failed for article ${job.data.articleId}: ${error.message}`,
    );
  }
}

...
export class AutomationService {
    ...
    async enhanceAllOriginalArticles(): Promise<{
        ...
        try {
            ...
            const articles = await this.articlesService.findAll({ status: 'ORIGINAL' });
            ...
            for (const article of articles) {
                ...
                jobIds.push(job.id);
                this.logger.log(`Queued enhancement job ${job.id} for article ${article.id}`);
            }
            ...
            this.logger.error(`Failed to queue enhancement jobs: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async enhanceArticle(articleId: string): Promise<{
        ...
        try {
            ...
            this.logger.log(`Queued enhancement job ${job.id} for article ${articleId}`);
            ...
            this.logger.error(`Failed to queue enhancement job: ${error.message}`, error.stack);
            throw error;
        }
    }

...
export class OpenAIProvider implements LLMProvider {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        this.model = this.configService.get<string>('llm.openai.model', 'gpt-4-turbo-preview');
        this.temperature = this.configService.get<number>('llm.openai.temperature', 0.7);
        this.maxTokens = this.configService.get<number>('llm.openai.maxTokens', 4000);
    }
    ...
    async enhance(prompt: string): Promise<string> {
        ...
        try {
            ...
            this.logger.error(`OpenAI API call failed: ${error.message}`, error.stack);

...
export class ScraperService {
     ...
     */
    async scrapeBeyondChats(): Promise<{ message: string; count: number; articleIds: string[] }> {
        const url = 'https://beyondchats.com/blogs';
        const maxArticles = this.configService.get<number>('scraping.maxArticles', 5);

        this.logger.log(`Starting BeyondChats scraping for ${maxArticles} articles`);
        ...
        try {
            ...
            const scrapedArticles = await strategy.scrape(url, {
                ...
                headless: this.configService.get<boolean>('scraping.puppeteerHeadless', true),
            });
     ...
     */
    async scrapeArticle(url: string): Promise<{ message: string; articleId: string }> {
        ...
        try {
            ...
            this.logger.error(`Failed to scrape article: ${error.message}`, error.stack);
            throw error;
        }
    }

...
import { ScraperStrategy, ScraperOptions, ScrapedArticle } from './scraper.interface';
...
export class BeyondChatsStrategy implements ScraperStrategy {
    ...
    async scrape(url: string, options?: ScraperOptions): Promise<ScrapedArticle[]> {
        ...
        this.logger.log(`Starting scrape of ${url} for ${maxArticles} oldest articles`);
    ...
    private async findLastPage(page: Page): Promise<number> {
        try {
            ...
            for (const selector of paginationSelectors) {
                ...
                if (lastPageElement) {
                    const lastPageText = await page.evaluate((el) => el.textContent, lastPageElement);
    ...
    private async navigateToPage(page: Page, pageNumber: number): Promise<void> {
        try {
            ...
            this.logger.error(`Navigation to page ${pageNumber} failed: ${error.message}`);
            throw error;
        }
    }
    ...
    private async getCurrentPageNumber(page: Page): Promise<number> {
        try {
            ...
            for (const selector of activePageSelectors) {
                ...
                if (activeElement) {
                    const pageText = await page.evaluate((el) => el.textContent, activeElement);
    ...
    private async extractArticles(page: Page, maxArticles: number): Promise<ScrapedArticle[]> {
        try {
            ...
            for (const selector of articleSelectors) {
                ...
                if (articleElements && articleElements.length > 0) {
                    this.logger.log(`Found ${articleElements.length} articles using selector: ${selector}`);
                    break;
                }
            }
            ...
            for (let i = 0; i < articlesToProcess; i++) {
                ...
                try {
                    ...
                    this.logger.warn(`Failed to extract article ${i + 1}: ${error.message}`);
                }
            }
